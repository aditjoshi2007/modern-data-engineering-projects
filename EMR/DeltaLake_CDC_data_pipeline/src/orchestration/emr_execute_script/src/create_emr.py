...
def get_initial_inputs(env, emr_name, local_dir, environment):
    ...
    if env in ['dev','tst']:
        alias = f'alias/<kms_key_alias>-{env}'
    else:
        alias = f'alias/<kms_key_alias>-{env}'
        
    response = client.describe_key(KeyId=f'{alias}')
    kms_cmk_arn = response['KeyMetadata']['Arn']

    configuration_file = f'{local_dir}/configs/' + environment + '/initial_inputs.json'
    initial_inputs = None
...
def get_instance_fleets(cluster_size, local_dir, environment):
    configuration_file = f'{local_dir}/configs/' + environment + '/instance_fleets.json'
