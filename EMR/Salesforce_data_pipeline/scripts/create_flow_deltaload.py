...
def create_flow(region_name,flow,object_name,buck_name,buck_pre,out_typ,
    ...
    try:
        if object_name.lower() == 'tbl1' or object_name.lower() == 'tbl2':
            datetimeTypeFieldName = 'col1'
        else:
            datetimeTypeFieldName = 'col2'
        config = boto3_config(
            retries={
                'max_attempts': 30,
                'mode': 'standard'
        ...
        flowName=flow,
        description='creating_delta_flows_using_csv',
        kmsArn = kms_cmk_arn,
            triggerConfig={
...
if __name__=='__main__':
    try:
        ...
        for index in range(len(object_properties)):
            flow.append(object_properties[index]['flow_common_name']+"_delta")
