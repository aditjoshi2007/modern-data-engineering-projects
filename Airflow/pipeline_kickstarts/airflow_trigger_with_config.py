...
def trigger_dag(dag_name, dag_conf, mwaa_env_name):
    mwaa_cli_command='dags trigger'
    client=boto3.client('mwaa', '<aws_region>')
    mwaa_cli_token=client.create_cli_token(Name=mwaa_env_name)
    conf=json.loads(dag_conf)  # Converting this str -> dict {"table_category": "bal", "schema_name": "dbsystt"}
...
def main(env, sp_name, dag_name, dag_conf, mwaa_env_name):
    ...
    port=data[env]["port"]
    status_code, err_message, out_message, execution_date=trigger_dag(dag_name, dag_conf, mwaa_env_name)
    print(f"***** {status_code}")
    print(f"***** {err_message}")
    print(f"***** {out_message}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dag_conf', help="Use dag conf as needed. Required Field", required=True)
    ...
    if args.dag_conf is None:
        print("dag_conf is required. Use dag conf as needed")
        raise MissingArgError
    else:
        dag_conf=args.dag_conf
    ...
    main(env, sp_name, dag_name, dag_conf, mwaa_env_name)
